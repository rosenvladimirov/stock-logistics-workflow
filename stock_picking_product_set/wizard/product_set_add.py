...
class PickingProductSetAdd(models.TransientModel):
    ...
    def _compute_product_set_line_ids(self):
        ...
        for rec in self:
            ...
            if lines:
                ...
                # Fallback to all lines from the current set
                rec.product_set_line_ids = rec.product_set_id.set_line_ids

    def _check_partner(self):
        """Validate order partner against the product set's partner if any."""
    ...
    def add_set(self):
        """Add product set, multiplied by quantity in the stock move line"""
        self._check_partner()
